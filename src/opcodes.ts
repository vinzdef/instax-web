/**
 * Opcodes accepted by the INSTAX BLE protocol.
 *
 * Reverse-engineered by the InstaxBLE project (https://github.com/javl/InstaxBLE);
 * only a subset is implemented here. Values are the 16-bit big-endian opcode
 * carried in bytes 4-5 of every packet.
 */
export const Opcode = {
  ...
  AR_LED_VIBRATION_SETTING: 0x3006,
...
/** Sub-command byte used with {@link Opcode.SUPPORT_FUNCTION_INFO}. */
...
const OPCODE_NAMES: ReadonlyMap<number, string> = new Map(
  Object.entries(Opcode).map(([name, value]) => [value as number, name]),
)

/** Human-readable name for an opcode, or `0x----` if unknown. */
export function opcodeName(opcode: number): string {
  return OPCODE_NAMES.get(opcode) ?? `0x${opcode.toString(16).padStart(4, '0')}`
}
